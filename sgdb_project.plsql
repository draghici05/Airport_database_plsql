...
DECLARE 
    V_CONTRACT_ID CONTRACTS.CONTRACT_ID%TYPE := 13;
    V_AIRLINE_ID CONTRACTS.AIRLINE_ID%TYPE := 13;
    V_AIRPORT_ID CONTRACTS.AIRPORT_ID%TYPE := 13;
...
DECLARE 
    ...
    V_HIRE_DATE DATE := TO_DATE('06/05/2024', 'DD-MM-YYYY');
...
-- 2. Give up on seat with id 6 

CREATE OR REPLACE PROCEDURE CANCEL_PASSENGER_SEAT ( P_PASSENGER_ID IN PASSENGERS.PASSENGER_ID%TYPE,
P_FIRST_NAME IN PASSENGERS.PASS_FIRST_NAME%TYPE, P_LAST_NAME IN PASSENGERS.PASS_LAST_NAME%TYPE) AS
BEGIN
    DELETE FROM PASSENGERS WHERE PASSENGER_ID = P_PASSENGER_ID AND PASS_FIRST_NAME = P_FIRST_NAME AND PASS_LAST_NAME = P_LAST_NAME;
    DBMS_OUTPUT.PUT_LINE('Passenger - ' || P_FIRST_NAME || ' ' || P_LAST_NAME || ' has given up the seat on ID ' || P_PASSENGER_ID);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Passenger - ' || P_FIRST_NAME || ' ' || P_LAST_NAME || ' cant be found ');
END;
/

BEGIN
    CANCEL_PASSENGER_SEAT(P_PASSENGER_ID => 6, P_FIRST_NAME => 'Nestor', P_LAST_NAME => 'Kelly');
END;
/

-- 3. Update contract value where id is 7 

CREATE OR REPLACE PROCEDURE UPDATE_CONTRACT_VAL (P_CONTRACT_ID IN CONTRACTS.CONTRACT_ID%TYPE, P_VALUE IN CONTRACTS.CONTRACT_VALUE%TYPE) AS
BEGIN
    UPDATE CONTRACTS SET CONTRACT_VALUE = P_VALUE WHERE CONTRACT_ID = P_CONTRACT_ID;
    DBMS_OUTPUT.PUT_LINE('The value for contract with id ' || P_CONTRACT_ID || ' has been updated');
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Contract does not exist');
END;
/

BEGIN
    UPDATE_CONTRACT_VAL(7, 2410.15);
END;
/
...
CREATE OR REPLACE PACKAGE P_PACKAGE AS 
    -- GET FLIGHT DETAILS
    FUNCTION FLIGHT_DETAILS (P_FLIGHT_ID IN FLIGHTS.FLIGHT_ID%TYPE) RETURN VARCHAR2;
    -- COUNT FLIGHTS
    FUNCTION COUNT_FLIGHTS RETURN NUMBER;
    -- DELETE FIGHT
    PROCEDURE DEL_FLIGHT (P_FLIGHT_ID IN FLIGHTS.FLIGHT_ID%TYPE);
END P_PACKAGE;
/

CREATE OR REPLACE PACKAGE BODY P_PACKAGE AS
    FUNCTION FLIGHT_DETAILS (P_FLIGHT_ID IN FLIGHTS.FLIGHT_ID%TYPE) RETURN VARCHAR2 IS
    V_FLIGHT_NUMBER FLIGHTS.FLIGHT_NUMBER%TYPE;
    V_DEPARTURE FLIGHTS.DEPARTURE_AIRPORT%TYPE;
    V_ARRIVAL FLIGHTS.ARRIVAL_AIRPORT%TYPE;
    V_FLIGHT_DETAILS VARCHAR2(200);
BEGIN
    SELECT FLIGHT_NUMBER, DEPARTURE_AIRPORT, ARRIVAL_AIRPORT
    INTO V_FLIGHT_NUMBER, V_DEPARTURE, V_ARRIVAL
    FROM FLIGHTS
    WHERE FLIGHT_ID = P_FLIGHT_ID;
    V_FLIGHT_DETAILS := 'Flight number - ' || V_FLIGHT_NUMBER || ' Departure Airport - ' || V_DEPARTURE || ' Arrival Airport - ' || V_ARRIVAL;
    RETURN V_FLIGHT_DETAILS;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('There are no details for this flight');
        RETURN NULL;
    END FLIGHT_DETAILS;

    FUNCTION COUNT_FLIGHTS RETURN NUMBER IS 
        V_COUNT NUMBER;
BEGIN
    SELECT COUNT(*) INTO V_COUNT FROM FLIGHTS;
    RETURN V_COUNT;
END COUNT_FLIGHTS;

    PROCEDURE DEL_FLIGHT (P_FLIGHT_ID IN FLIGHTS.FLIGHT_ID%TYPE) IS 
BEGIN 
    DELETE FROM FLIGHTS WHERE FLIGHT_ID = P_FLIGHT_ID;
    IF SQL%ROWCOUNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('There are no flights to be deleted');
    ELSE 
...
DECLARE 
    V_FLIGHT_ID NUMBER := 5;
    V_FLIGHT_DETAILS VARCHAR2(200);
    V_COUNT_FLIGHTS NUMBER;
BEGIN
    V_FLIGHT_DETAILS := P_PACKAGE.FLIGHT_DETAILS(V_FLIGHT_ID);
    DBMS_OUTPUT.PUT_LINE('The flight details are: ' || V_FLIGHT_DETAILS);
    
    V_COUNT_FLIGHTS := P_PACKAGE.COUNT_FLIGHTS;
    DBMS_OUTPUT.PUT_LINE('Total number of flights: '|| V_COUNT_FLIGHTS);
    
    P_PACKAGE.DEL_FLIGHT(V_FLIGHT_ID);
END;
/

-- G. Triggers at statement and row level

-- Triggers at statement level 
-- Check if airplanes have been bought a long time ago

CREATE OR REPLACE TRIGGER STATEMENT_TRIGGER1 
BEFORE INSERT ON AIRPLANES
FOR EACH ROW
BEGIN
    IF :NEW.PRODUCTION_YEAR < 2016 THEN 
        RAISE_APPLICATION_ERROR(-20101, 'Airplane is too old');
    END IF;
END;
/


-- Triggers at row level 
...
BEGIN   
    SELECT COUNT(*) INTO V_CAPACITY FROM PASSENGERS WHERE FLIGHT_ID = :NEW.FLIGHT_ID;
    SELECT AIRPLANE_CAPACITY INTO V_MAX_CAPACITY FROM AIRPLANES WHERE AIRPLANE_ID = (SELECT AIRPLANE_ID FROM FLIGHTS WHERE FLIGHT_ID = :NEW.FLIGHT_ID);
    IF V_CAPACITY >= V_MAX_CAPACITY THEN
        RAISE_APPLICATION_ERROR(-20103, 'Flight has reached max capacity');
    END IF;
END;
/